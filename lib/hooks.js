...
export function useListBannersQuery() {
  return useQuery(["banners"], () => listBanners());
}

export function useListPhonesQuery() {
  return useQuery(["phones"], () => listPhones(), { staleTime: 5 * 60 * 1000 });
}

export function useListAgenciesQuery() {
  return useQuery(["agencies"], () => listAgencies(), { staleTime: 5 * 60 * 1000 });
}

export function useGetLiquidacionInfoQuery(license_plate) {
  return useQuery(
    ["liquidacionInfo", license_plate],
    () => getLiquidacionInfo(license_plate),
    { enabled: !!license_plate }
  );
}
...
export function useListRangeQuery() {
  return useQuery(["ranges"], () => listRanges(), {
    staleTime: 5 * 60 * 1000,
  });
}
