...
export const HomeContactSchema = Yup.object().shape({
  ...
  aceptaPolitica: Yup.boolean()
    .oneOf([true], "Debes aceptar la Política de Protección de Datos para continuar")
    .required(),
  autorizaDatos: Yup.boolean()
    .oneOf([true], "Debes autorizar el envío de información comercial para continuar")
    .required(),
});
...
export const RaceTrackSchema = Yup.object().shape({
  name: Yup.string().required("Campo requerido"),
  lastname: Yup.string().required("Campo requerido"),
  phone: Yup.string()
    .test("Sólo dígitos", "El campo debe tener solo dígitos", digitsOnly)
